...
$version = \OC::$server->getAppManager()->getAppVersion('custome');
$uid = \OC::$server->getSession()->get('user_id');

?>

<!DOCTYPE html>
<html class="ng-csp" data-placeholder-focus="false" lang="<?php

															use function Sabre\Event\Loop\clearInterval;
															use function Sabre\Event\Loop\setInterval;

															p($_['language']); ?>" data-locale="<?php p($_['locale']); ?>">
...
<head data-user="<?php p($_['user_uid']); ?>" data-user-displayname="<?php p($_['user_displayname']); ?>" data-requesttoken="<?php p($_['requesttoken']); ?>">
	...
	<?php
	$uid = \OC::$server->getSession()->get('user_id');
	if ($uid == 'Sysadmin@2021' || $uid == 'Secadmin@2021' || $uid == 'Audadmin@2021') {
		$styles = '<style>';
		$styles .= '#controls .button.new,';
		$styles .= '#filestable,';
		$styles .= '#recommendations { display: none!important; }';
		$styles .= '</style>';
		echo $styles;
	}
	?>
...
<body id="<?php p($_['bodyid']); ?>">
				...
				<nav class="settings-menu" id="expanddiv" style="display:none;" aria-label="<?php p($l->t('Settings menu')); ?>">
					<ul>
						<?php foreach ($_['settingsnavigation'] as $entry) : ?>
							<li data-id="<?php p($entry['id']); ?>" id="expanddiv-child-<?php p($entry['id']); ?>">
	...
	<?php if ($uid == 'Audadmin@2021') : ?>
		<script nonce="<?php p(\OC::$server->getContentSecurityPolicyNonceManager()->getNonce()) ?>" src="/core/js/custome/2021-09-05/remove-button.js?v=<?php p($version) ?>"></script>
	<?php endif ?>

	<?php if ($uid == 'Audadmin@2021' || $uid == 'Secadmin@2021' || $uid == 'Sysadmin@2021') : ?>
		<script nonce="<?php p(\OC::$server->getContentSecurityPolicyNonceManager()->getNonce()) ?>" src="/core/js/custome/2021-09-05/add-hint.js?v=<?php p($version) ?>"></script>
	<?php endif ?>
</html>